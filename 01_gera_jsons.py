...
PARENT_DIR = os.path.join(BASE_DIR, 'correcao_s4')        # pasta com subpastas regionais
OUTPUT_JSON_DIR = os.path.join(BASE_DIR, 'json_correcao')  # pasta para salvar os JSONs
MANUAL_COMP_DIR_SUFFIX = '_comp_manual_s4'                # sufixo para pasta compensação manual
...
def processar_imagem(image_path):
    ...
    x_coords = [215, 758, 1298+SHIFT, 1840+SHIFT2]
    y_coords = [(1820+15) + 87 * i for i in range(11)]
    RECT_SIZE = (50, 50)
    quest = {}
    for col_idx, x_base in enumerate(x_coords):
        for row_idx, y_base in enumerate(y_coords):
            ...
            for opt in range(4):
                px = x_base + opt*100
                p, s = analisar_retangulo(gray, px, y_base, RECT_SIZE[0], RECT_SIZE[1], threshold=28)
                if s:
                    marked.append(opt)
