...
PARENT_DIR             = os.path.join(BASE_DIR, "contagem")      # raiz das regiões
OUTPUT_JSON_DIR        = os.path.join(BASE_DIR, "json_5ano")        # onde salvar .json
...
def query_table(schema: str, sim_prefix: str) -> pd.DataFrame:
    ...
    df = pd.read_sql(f"SELECT * FROM {table};", engine, dtype={'estudante_id': str, "simulado_id":str})
...
def processar_imagem_json(image_path: str) -> dict:
    ...
    if 'estudante_id' in qr:
        df_av = query_table(SCHEMA, SIMULADO)          # apenas ids terminados em 05/09
...
import concurrent.futures, json, shutil, time
...
def main() -> None:
    ...
    for region in [d for d in os.listdir(PARENT_DIR)
                   if os.path.isdir(os.path.join(PARENT_DIR, d))]:
        region_dir = os.path.join(PARENT_DIR, region)

        # se existirem subpastas com imagens, usa-as; senão, usa a própria região
        ...
        # pasta para copiar imagens cujo QR não foi lido
        ...
        for img_dir in img_dirs:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
                futuros = {pool.submit(processar_imagem_json,
                                       os.path.join(img_dir, im)): im
                           for im in imagens}

                for fut in concurrent.futures.as_completed(futuros):
                    nome = futuros[fut]
                    try:
                        dado = fut.result()          # dict retornado pela função
                        if not dado:                 # None → erro de leitura
                            continue

                        # chave = dump ordenado do dict qrcode
                        chave = json.dumps(dado["qrcode"], sort_keys=True,
                                           ensure_ascii=False)
                        resultados[chave] = dado

                        # copia imagem se QR não reconhecido
                        if dado["qrcode"].get("raw") == "Unknown":
                            src = os.path.join(img_dir, nome)
                            dst = os.path.join(comp_manual, nome)
                            shutil.copy(src, dst)

                    except Exception as exc:
                        print(f"[ERRO] {nome}: {exc}")

        # grava JSON da região
        saida = os.path.join(OUTPUT_JSON_DIR,
                             f"{region.replace(' ', '_')}.json")
        with open(saida, "w", encoding="utf-8") as fp:
            json.dump(resultados, fp, ensure_ascii=False, indent=2)
        print(f"✓ Região '{region}' concluída → {saida}")

    print(f"Tempo total: {time.perf_counter() - tic:.2f}s")
